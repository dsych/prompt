...
prompt.start = function (options) {
  ...
  // check for the key combinations to listen for.
  // it could be an Array of string or a single string.
  // NOTE: for the Ctrl or Shift key use the followig pattern
  // { name: "", key: "", ctrl: bool, shift: bool }
  if (options.keyCombinations){
    if (Array.isArray(options.keyCombinations)){
      options.keyCombinations.forEach(function (el) {
          stdin.on('keypress', function (char, key) {
              if(key && key.ctrl == el.ctrl && key.shift == el.shift && key.name == el.key){
                  process.emit(el.name);
              }
          });
      });
    } else {
        stdin.on('keypress', function (char, key) {
            if(key && key.ctrl == options.keyCombinations.ctrl && key.shift == el.shift && key.name == options.keyCombinations.key){
                process.emit(options.keyCombinations.name);
            }
        });
    }
  }
